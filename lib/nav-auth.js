...
import { supabase, currentProfile, clearBlockFilterCache } from "./supabase.js?v=5";
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("/sw.js")
      .then((reg) => {
        ...
        // cc-v209: when the SW detects a new version after install,
        // surface a small "tap to refresh" banner so users can reload
        // and pick up the new HTML/JS without manually hard-refreshing.
        // Without this, sw.js skipWaiting() takes over future requests
        // but the currently-loaded page keeps running its in-memory
        // (old) code until reload. Recurring source of "I deployed but
        // the new feature doesn't show up" confusion.
        if (reg) {
          reg.addEventListener("updatefound", () => {
            ...
            newWorker.addEventListener("statechange", () => {
              if (newWorker.state === "installed" && navigator.serviceWorker.controller) {
                // Only show the banner when there was ALREADY a controlling
                // SW (i.e. this is a real UPDATE, not the first-ever install
                // on this device). Initial installs don't need a reload.
                showSwUpdateBanner();
              }
            });
          });
...
// Update-available banner. Pinned to the top of the viewport with
// brand-gradient styling, click to reload. De-duped via the element id
// so subsequent updatefound events don't stack copies on top of each
// other. Intentionally NOT auto-reloading because the user might be
// mid-form (new listing, contact modal, Collection item modal) and
// losing that state mid-edit is worse than waiting for them to tap.
function showSwUpdateBanner() {
  if (document.getElementById("cc-sw-update-banner")) return;
  const banner = document.createElement("div");
  banner.id = "cc-sw-update-banner";
  banner.setAttribute("role", "alert");
  banner.style.cssText = [
    "position: fixed",
    "top: 0", "left: 0", "right: 0",
    "background: linear-gradient(90deg, #FF6A1A, #E91E63, #8B1FC4, #2D7FFF)",
    "color: #ffffff",
    "padding: 10px 14px",
    "text-align: center",
    "z-index: 99999",
  ...
  banner.innerHTML = "🔄 New CanvasCircle version available, tap to refresh";
  banner.addEventListener("click", () => {
    // Defensive: if the user taps multiple times, only reload once.
    if (banner.dataset.reloading === "1") return;
    banner.dataset.reloading = "1";
...
(async function decorateAuthLinks() {
  ...
  supabase.auth.onAuthStateChange((_event, _session) => {
    // Drop the per-tab block-filter cache whenever auth state changes — the
    // signed-in user just changed (or signed out), so the cached Set is stale.
    try { clearBlockFilterCache(); } catch {}
    update();
  });
})();
