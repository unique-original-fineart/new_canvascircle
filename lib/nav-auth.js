...
if (!document.getElementById("nav-auth-styles")) {
  ...
  style.textContent = `
    /* Signed-in "My Account" link — styled to match the other nav links
       (Catalog, Guidelines, About). The ↗ indicator stays so users see
       this link opens a separate section (the portal), but the pill
       background is gone — gradient treatment is reserved for higher-
       signal elements (Verified, Established, active page underline). */
    ...
    @media (max-width: 640px) {
      ...
      /* My Account link: the base rule sets display:inline-flex which makes
         the link shrink to its content width even inside a flex column
         container. On mobile we need it to stretch like the other rows so
         the active-state gradient bar at left:0 lines up with the menu's
         left edge instead of overlapping the "M" of "My Account". */
