...
export function showAuthPrompt(opts = {}) {
  ...
  // Arm the return trail right before showing — so the modal's "Sign in" /
  // "Create account" links carry the user back to wherever they are now
  // after they're authenticated. The caller can pass `opts.returnLabel` to
  // customize what shows up in the post-signin "Continue to X" banner.
  armReturnTrail(opts.returnLabel || "the page you were on");
