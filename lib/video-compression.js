...
//   - Safari (iOS + macOS): supported on 14.1+ (2021) — but with one
//     known gap: iOS Safari (even on iOS 18) does NOT implement
//     HTMLMediaElement.captureStream(). That means we can't pass the
//     original audio track through on iOS — the compressed output is
//     silent. Video frames are unaffected; verification still works
//     fine since the handwritten card + visible artwork are the
//     load-bearing evidence. Audio is bonus. On Chrome/Firefox/Android
//     audio is preserved as before.
//
